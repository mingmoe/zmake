...
impl NativeModuleLoader for NetModule {
    ...
    fn get_module_exports(
        ...
        exports.push(("readFile",q_ctx.create_function_async("readFileAsync", async |this,args|{
            let path = args.get(0).unwrap().get_str();
            let contents = tokio::fs::read_to_string(path).await.unwrap();
            Ok(JsValueFacade::new_string(contents))
        }, 1).unwrap()));
        
        exports.push(("writeFile",q_ctx.create_function_async("readFileAsync", async |this,args|{
            let path = args.get(0).unwrap().get_str();
